...
// 设置静态目录
// /static/file 伪路径
app.use('/static/file', express.static(__basename + '/upload'))

// 解析post请求体
// 将post请求参数解析为json
// limit限制解析post请求体大小 如果不设置, 可能会因为请求体太大而被拦截
...
app.listen(config.serverOptions.port, () => {
    console.log(`running in ${config.serverOptions.host}:${config.serverOptions.port}`);
})